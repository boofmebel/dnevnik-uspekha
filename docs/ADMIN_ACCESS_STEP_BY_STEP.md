# 🎯 Получение доступа к админке - Пошаговая инструкция

## ✅ Самый простой способ (если у вас уже есть аккаунт)

### Шаг 1: Подключитесь к базе данных

```bash
ssh root@89.104.74.123
```

Затем найдите способ подключиться к PostgreSQL. Попробуйте:

```bash
# Вариант 1
psql -U postgres -d dnevnik_uspekha

# Вариант 2 (если нужен пароль)
sudo -u postgres psql -d dnevnik_uspekha

# Вариант 3 (если база в Docker)
docker exec -it <container_name> psql -U postgres -d dnevnik_uspekha
```

### Шаг 2: Обновите роль пользователя

```sql
-- Замените на ваш email
UPDATE users SET role = 'admin' WHERE email = 'ваш_email@example.com';
```

### Шаг 3: Проверьте

```sql
SELECT email, role FROM users WHERE email = 'ваш_email@example.com';
```

Должно быть: `role = 'admin'`

### Шаг 4: Войдите в админку

1. Откройте: `http://89.104.74.123:3000`
2. Войдите с вашим email и паролем
3. Откройте: `http://89.104.74.123:3000/admin.html`

---

## 🔐 Создание нового администратора

### Шаг 1: Сгенерируйте хеш пароля

**Вариант A: Онлайн генератор**
1. Откройте: https://bcrypt-generator.com/
2. Введите пароль
3. Скопируйте хеш (например: `$2b$12$...`)

**Вариант B: Локально (если установлен Python)**
```bash
pip install passlib[bcrypt]
python3 -c "from passlib.context import CryptContext; pwd_context = CryptContext(schemes=['bcrypt'], deprecated='auto'); print(pwd_context.hash('ваш_пароль'))"
```

### Шаг 2: Подключитесь к базе данных

(см. Шаг 1 выше)

### Шаг 3: Создайте администратора

```sql
INSERT INTO users (email, password_hash, role, created_at)
VALUES (
  'admin@dnevnik-uspekha.ru',
  '$2b$12$...',  -- Вставьте хеш из шага 1
  'admin',
  NOW()
);
```

### Шаг 4: Войдите в админку

(см. Шаг 4 выше)

---

## 🔍 Как найти параметры подключения к базе

### Вариант 1: Проверьте .env файл

```bash
ssh root@89.104.74.123
cd /var/www/dnevnik-uspekha
find . -name '.env' -o -name '*.env' 2>/dev/null
cat .env | grep DATABASE
```

### Вариант 2: Проверьте конфигурацию backend

```bash
ssh root@89.104.74.123
cd /var/www/dnevnik-uspekha
find . -name 'config.py' -path '*/core/*' 2>/dev/null
```

### Вариант 3: Проверьте Docker

```bash
ssh root@89.104.74.123
docker ps
docker inspect <container_name> | grep -i postgres
```

---

## 📋 Примеры SQL команд

### Обновить существующего пользователя:
```sql
UPDATE users SET role = 'admin' WHERE email = 'user@example.com';
```

### Создать нового администратора:
```sql
INSERT INTO users (email, password_hash, role, created_at)
VALUES ('admin@example.com', '$2b$12$...', 'admin', NOW());
```

### Проверить всех администраторов:
```sql
SELECT id, email, role, created_at FROM users WHERE role = 'admin';
```

### Проверить всех пользователей:
```sql
SELECT id, email, role FROM users ORDER BY created_at DESC;
```

---

## ⚠️ Если не получается подключиться к базе

1. **Проверьте, запущена ли база данных:**
   ```bash
   systemctl status postgresql
   # или
   docker ps | grep postgres
   ```

2. **Проверьте, на каком порту работает PostgreSQL:**
   ```bash
   netstat -tlnp | grep 5432
   # или
   ss -tlnp | grep 5432
   ```

3. **Проверьте логи:**
   ```bash
   journalctl -u postgresql -n 50
   # или
   docker logs <container_name>
   ```

---

## ✅ После успешного создания администратора

1. ✅ Войдите на главной странице: `http://89.104.74.123:3000`
2. ✅ Откройте админку: `http://89.104.74.123:3000/admin.html`
3. ✅ Должен появиться интерфейс админки с:
   - Тёмным заголовком "🔐 Админ-панель"
   - Навигацией: Dashboard, Пользователи, Дети, Подписки, Уведомления
   - Статистикой в карточках

---

## 📞 Нужна помощь?

См. другие инструкции:
- `docs/CREATE_ADMIN_NOW.md` - Быстрая инструкция
- `docs/HOW_TO_GET_ADMIN_ACCESS.md` - Полная инструкция
- `docs/ADMIN_TROUBLESHOOTING.md` - Решение проблем

