...
// При изменении структуры данных увеличивайте версию
...
function migrateData(data, fromVersion, toVersion) {
  ...
  // Если версия не указана, считаем что это версия 1.0.0
  if (!fromVersion) {
    fromVersion = '1.0.0';
  }
  
  // Если версия уже актуальна, возвращаем данные как есть
  ...
  if (window.errorHandler) {
    window.errorHandler.log(window.errorHandler.LogLevel.INFO, 'Migrating data', {
      fromVersion,
      toVersion
    });
  }
  ...
  if (fromVersion === '1.0.0' && toVersion === '2.0.0') {
    ...
    // Инициализируем wallet если отсутствует
    if (!data.wallet) {
      data.wallet = {
        amount: 0,
        history: []
    ...
    // Инициализируем profile.gender если отсутствует
    if (data.profile && !data.profile.gender) {
      data.profile.gender = 'none';
    }
    
    // Инициализируем stars.claimedRewards если отсутствует
    if (data.stars && !data.stars.claimedRewards) {
      data.stars.claimedRewards = [];
    }
  }
  
  // Здесь можно добавить другие миграции для будущих версий
...
function loadData() {
  ...
  if (saved) {
    ...
    if (!parsed) {
      ...
      if (window.errorHandler) {
        window.errorHandler.log(window.errorHandler.LogLevel.ERROR, 'Failed to parse saved data');
      }
      return;
    }
    
    try {
      // Получаем версию загруженных данных
      const loadedVersion = parsed.version || '1.0.0';
      
      // Выполняем миграцию если версия отличается
      if (loadedVersion !== CURRENT_DATA_VERSION) {
        parsed = migrateData(parsed, loadedVersion, CURRENT_DATA_VERSION);
      }
      ...
      appData = {
        ...appData,
        ...parsed,
        version: CURRENT_DATA_VERSION, // Всегда используем текущую версию
        stars: {
          ...appData.stars,
          ...(parsed.stars || {}),
      ...
      if (window.errorHandler) {
        window.errorHandler.log(window.errorHandler.LogLevel.ERROR, 'Failed to load data', {
          error: e.message,
          stack: e.stack
        });
      }
      // Восстанавливаем дефолтные данные при критической ошибке
      appData = { ...appData, version: CURRENT_DATA_VERSION };
    }
  } else {
    // Если данных нет, устанавливаем текущую версию
    appData.version = CURRENT_DATA_VERSION;
  }
}
...
function saveData() {
  try {
    ...
    if (!appData || typeof appData !== 'object') {
      ...
      if (window.errorHandler) {
        window.errorHandler.log(window.errorHandler.LogLevel.ERROR, 'Invalid data structure for save');
      }
      return false;
    }
    
    // Убеждаемся, что версия установлена
    appData.version = CURRENT_DATA_VERSION;
    ...
    if (!sizeCheck.valid) {
      ...
      if (window.errorHandler) {
        window.errorHandler.log(window.errorHandler.LogLevel.ERROR, 'Data size check failed', {
    ...
    if (!jsonString || jsonString === 'null' || jsonString === 'undefined') {
      ...
      if (window.errorHandler) {
        window.errorHandler.log(window.errorHandler.LogLevel.ERROR, 'Failed to serialize data');
      }
      return false;
    }
    
    // Используем errorHandler для обработки ошибок localStorage
    try {
      localStorage.setItem('responsibilityAppData', jsonString);
      if (window.errorHandler) {
        window.errorHandler.log(window.errorHandler.LogLevel.DEBUG, 'Data saved successfully', {
          size: sizeCheck.size,
          version: CURRENT_DATA_VERSION
        });
      }
      ...
      const result = window.errorHandler ? 
        window.errorHandler.handleStorageError(storageError, 'saveData') : 
    ...
    if (window.errorHandler) {
      window.errorHandler.log(window.errorHandler.LogLevel.ERROR, 'Unexpected error in saveData', {
        error: error.message,
        stack: error.stack
      });
    }
